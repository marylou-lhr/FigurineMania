    ...
    <main class="flex-grow-1">
      <div class="container">
        ...
        <div class="card mb-3" style="max-width: 540px;">
          <div class="row g-0">
            <?php
              if (isset($_SESSION['panier'])) {
                $ids = array_filter(array_keys($_SESSION['panier']), 'is_numeric'); // Liste des identifiants des figurines
              } else {
                $ids = []; // Si le panier n'existe pas, on retourne un tableau vide
              }
              ...
              if (empty($ids)) { // Si la liste est vide
                ...
                foreach ($figurines as $figurine):
                  if (!isset($_SESSION['panier'][$figurine['id']])) {
                    continue; // Si la figurine n'est pas dans le panier, on passe à la suivante
                  }
                        ...
                        <div class="card-body">
                          ...
                          <p class="card-text">
                            <small class="text-body-secondary">
                              <?=$figurine['prix']?> € <br> Quantité : <?=$_SESSION['panier'][$figurine['id']]?>
                            </small>
                          </p>
          ...
          <?php if (!empty($ids)) { // Afficher le total et le bouton si le panier n'est pas vide ?>
          <br>
          <div class="row mt-4">
