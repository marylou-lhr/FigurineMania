<?php
  ...
  if (isset($_GET['btn-confirm']) &&
      isset($_GET['inputNom']) && 
      isset($_GET['inputLicense']) && 
      isset($_GET['inputImage']) && 
      isset($_GET['inputPrix'])){

    $jsonFile = 'figurines.json';
    $jsonData = file_get_contents($jsonFile);
    $figurines = json_decode($jsonData, true);
    $lastId = 0;
    foreach ($figurines as $figurine) {
      if (isset($figurine['id']) && $figurine['id'] > $lastId) {
          $lastId = $figurine['id'];
      }
    }
    ...
    $newFigurine = array(
      ...
      'nom' => $_GET['inputNom'],
      'prix' => $_GET['inputPrix'],
      'license' => $_GET['inputLicense'],
      'urlImage' => $_GET['inputImage']
    );

    // Ajouter la nouvelle figurine à la liste
    $figurines[] = $newFigurine;

    // Convertir les données mises à jour en JSON
    $newJsonData = json_encode($figurines, JSON_PRETTY_PRINT);
